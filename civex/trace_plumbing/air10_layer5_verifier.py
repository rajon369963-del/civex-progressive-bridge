...
def verify_trace(trace_id, target_stdout_file=None):
    ...
    if not rows:
        print(f"ERROR: No trace events found for trace_id={trace_id}", file=sys.stderr)
        conn.close()
        sys.exit(1)
    ...
    if not input_file or not os.path.isfile(input_file):
        print(f"ERROR: Input file cannot be resolved from execution details or argv", file=sys.stderr)
        conn.close()
        sys.exit(1)
    ...
    # 1. Strict RFC 8259 verification
    ...
    # Append immutable summary to tool_traces_v2
    ...
    cur.execute("""
        ...
        VALUES (?, ?, 'TASK_CLI_HOTPATH', ?, ?, ?, 'BOUNDARY_INTERCEPTED', ?, ?, 'STDOUT_RECORDED', ?, ?, ?, ?, ?, ?)
    """, (
        trace_id, task_intent, candidates_json, chosen_tool, target_bin,
        input_file, current_input_sha256, actual_returncode, duration_ms,
        semantic_result, verdict_status, failure_reason, now_iso
    ))
